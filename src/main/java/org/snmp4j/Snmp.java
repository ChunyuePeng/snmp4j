 ...
 * Snmp类是SNMP4J的核心之一。提供方法发送和接收SNMP PDU。所有SNMP PDU类型都能被发送。
 *惯常的PDU能被异步或同步发送
 * <p>
 ...
 * Snmp类是独立的传输协议，为特定TransportMapping实例支持通过调用TransportMapping类中的add
 * TransportMapping方法或通过使用非默认一致传输映射构造器创建一个Snmp实例。传输映射被用于新的和即将
 * 发出的message
 *
 * <p>
